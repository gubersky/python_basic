...
def decorator(func):
    def wrapper(*args, **kwargs):
        while True:
            print("Password must contains minimum 1 number, letter, "
                  "special character and length at least 8 characters")
            password = func(*args, **kwargs)
            if password is None:
                print("Wrong data: spaces and tabs are not accepted for "
                      "the password!\n")
            ...
            elif not any([1 if i in string.punctuation else 0 for
                          i in password]):
